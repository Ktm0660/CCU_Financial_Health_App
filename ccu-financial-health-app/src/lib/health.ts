export type Inputs = {
  ...
  debts: { minimumPayment: number }[];
  creditUtilization?: number | null;
};

export function calcDTI({ monthlyIncome, debts }: Inputs) {
  if (!monthlyIncome) return 1;
  const totalMin = debts.reduce((s, d) => s + (d.minimumPayment || 0), 0);
...
export function buildRecommendations({ dti, emerg, savings, creditUtil }:
  { dti: number; emerg: number; savings: number; creditUtil?: number | null; }): string[] {
  const recs: string[] = [];
  ...
  if (creditUtil != null && creditUtil > 0.3) recs.push("Keep credit utilization under 30% to strengthen credit health.");
